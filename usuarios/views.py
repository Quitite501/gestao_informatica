...
def usuario_novo(request):
    ...
    if form.is_valid():
        usuario = form.save()
        registrar_auditoria(request, RegistroAuditoria.ACAO_CRIACAO, "Usuario", usuario.pk, f"Usuário {usuario.nome_completo} criado.")
...
def usuario_editar(request, pk):
    ...
    if form.is_valid():
        usuario = form.save()
        registrar_auditoria(request, RegistroAuditoria.ACAO_EDICAO, "Usuario", usuario.pk, f"Usuário {usuario.nome_completo} editado.")
        return redirect("usuario_lista")
    return render(request, "usuarios/usuario_form.html", {
        "form": form,
...
def usuario_desativar(request, pk):
    ...
    if request.method == "POST":
        ...
        usuario.save()
        registrar_auditoria(request, RegistroAuditoria.ACAO_DESATIVACAO, "Usuario", usuario.pk, f"Usuário {usuario.nome_completo} desativado.")
        return redirect("usuario_lista")
